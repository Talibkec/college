<jsp:include page="/jsp/header.jsp" />


    <div class="jumbotron" style="padding-top: 24px; padding-bottom: 24px;">
        <div class="container">
            <div class="row">
                <div class="col-md-10 col-sm-8"><h3>Placement Brochure</h3></div>

            </div>
        </div>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <ul class="list-group page-menu">
                    <li class="list-group-item" style="background: #eee; font-size: 20px;">Training & Placement Cell
                    </li>
                    <li class="list-group-item"><a href="/trainingplacement/aboutplacement">&nbsp;&nbsp; <i
                            class="fa fa-angle-double-right"></i> About Placement</a></li>
                    <li class="list-group-item"><a href="/trainingplacement/rulesoftrainingplacement">&nbsp;&nbsp; <i
                            class="fa fa-angle-double-right"></i> Rules of Training & Placement</a></li>
                    <li class="list-group-item"><a href="/trainingplacement/tipsforresume">&nbsp;&nbsp; <i
                            class="fa fa-angle-double-right"></i> Tips for Resume</a></li>
                    <li class="list-group-item"><a href="/trainingplacement/placementbrochure">&nbsp;&nbsp; <i
                            class="fa fa-angle-double-right"></i> Placement Brochure</a></li>
                </ul>
            </div>
            <div class="col-md-8">
                <div class="page-content" style="line-height: 175%; font-size: 125%; font-weight: 300;">

                    <object data=" https://drive.google.com/viewerng/viewer?embedded=true&url=http://keck.ac.in/trainingplacement/brochure.pdf" type="application/pdf" width="700" height="1000">
                        <embed src="https://drive.google.com/viewerng/viewer?embedded=true&url=http://keck.ac.in/trainingplacement/brochure.pdf" type="application/pdf">
                    </object>
                </div>
            </div>
        </div>
    </div>

<jsp:include page="/jsp/footer.jsp" />
